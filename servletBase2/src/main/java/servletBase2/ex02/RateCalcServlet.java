...
public class RateCalcServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command !=null && command.equals("calculate")) {
			...
			out.print("<h3>"+result+" " + operator+"</h3>");
			out.print("<a href='/servletBase2/calc'>환율계산기");
			out.print("</body>");
			out.print("</html>");
		}else {
			out.print("<html><title>환율계산기</title>"	);
			...
			out.print("<option value='달러'>달러</option>");
			out.print("<option value='엔화'>엔화</option>");
			out.print("<option value='위환'>위환</option>");
			out.print("<option value='파운드'>파운드</option>");
			out.print("<option value='유로'>유로</option>");
			out.print("</select>");
	...
	private static String calculate(float won , String operator) {
		...
		if(operator.equals("달러")) {
			result=String.format("%.6f", won/USD_RATE); // %.6f : 소수이하 6자리
